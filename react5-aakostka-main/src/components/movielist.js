...
import styles from './movielist.module.css'; // Import the css rules.
        ...
        const MovieListCarousel = ({movieList}) => {
            ...
            return (
                <Carousel onSelect={this.handleSelect}>
                    {movieList.map((movie) =>
                        <Carousel.Item key={movie._id} className={styles.carouselItem}>
                            <div>
                                <LinkContainer to={'/movie/'+movie._id} onClick={()=>this.handleClick(movie)}>
                                    <Nav.Link><Image className={styles.image} src={movie.imageUrl} thumbnail /></Nav.Link>
                                </LinkContainer>
                            </div>
                            <Carousel.Caption>
                                <h4>{movie.title}</h4>
